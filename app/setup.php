...
*/
    ...
    $query = "CREATE TABLE Feedback( feedbackId INT(10) NOT NULL AUTO_INCREMENT, studentId INT NOT NULL ,employerId INT NOT NULL, 
              comments VARCHAR (1000),  isStudent BOOLEAN NOT NULL, PRIMARY KEY (feedbackId), 
             FOREIGN KEY (employerId) REFERENCES Employer(employerId) ON DELETE CASCADE  ON UPDATE CASCADE,
                FOREIGN KEY (studentId) REFERENCES Student(studentId) ON DELETE CASCADE  ON UPDATE CASCADE)";
    $ret = $connection->query ($query);

    $query = "CREATE TABLE Rating(rateId INT(10) NOT NULL AUTO_INCREMENT, studentId INT NOT NULL ,employerId INT NOT NULL,  
               starRank INT(10) NOT NULL,PRIMARY KEY (rateId), 
                FOREIGN KEY (employerId) REFERENCES Employer(employerId) ON DELETE CASCADE  ON UPDATE CASCADE,
                FOREIGN KEY (studentId) REFERENCES Student(studentId) ON DELETE CASCADE  ON UPDATE CASCADE)";
    $ret = $connection->query ($query);

    $query = "CREATE TABLE Chat(chatId INT(10) NOT NULL AUTO_INCREMENT,studentId INT NOT NULL ,employerId INT NOT NULL,
               timestamp TIMESTAMP  NOT NULL, message VARCHAR (2000),PRIMARY KEY (chatId),
               FOREIGN KEY (employerId) REFERENCES Employer(employerId) ON DELETE CASCADE  ON UPDATE CASCADE,
                FOREIGN KEY (studentId) REFERENCES Student(studentId) ON DELETE CASCADE  ON UPDATE CASCADE )";
     $ret = $connection->query ($query);
